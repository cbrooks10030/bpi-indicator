...
|---|---|---|---|---|---|
| F-01 | Critical | Breakers (`brk_`) | `request.security(..., brk_Fmtf(), lookahead = barmerge.lookahead_on)` requests current-bar HTF pivots with look-ahead enabled. Historical BOS/MSS marks and their 8 alerts appear earlier than they could in real time. Only manifests when `brk_tfStructure` is set above the chart timeframe — which the tooltip instructs users to do. | 3218–3231 | Open |
...
| F-09 | Medium | Alerts | The four Breakers `alert()` calls omit `freq`, defaulting to `alert.freq_once_per_bar` — they can fire on an unconfirmed bar. | 3288, 3290, 3330, 3332 | Open |
